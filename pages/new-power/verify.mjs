...
const CL030 = ['new power muay thai', 'new power muay thai event', 's new power muay thai'];
const CL035 = ['wednesday fight night', 'wednesday', 'wednesday night boxing', 'anything on wednesday', 'wednesday night'];
const DATED_KW = ['new power muay thai', 'wednesday 30 september 2026', 'rajadamnern stadium',
  'traditional five-round muay thai', 'new power muay thai tickets'];

/* Document B section 32: the specific claims to hunt for. */
const FORBIDDEN = [
  [/\bofficial\b/i, 'the word "official"'],
  [/fast-paced|knockout-focused|non-stop action|relentless|explosive momentum|electric roar/i, 'forbidden event characterisation'],
  [/\bchampion(ship)?\b|\btitle fight\b|elite fighter|famous fighter|international star/i, 'unverified fighter status'],
  [/instant ticket|arrives immediately|immediate e-ticket|automatic stadium ticket/i, 'instant-delivery claim'],
  [/22:00 finish|10 ?PM|6pm ?- ?10pm|four-hour/i, 'an invented finish time'],
  [/hear(ing)? (the )?(trainer|corner)|fight sounds|see(ing)? (the )?sweat|feel(ing)? (the )?impact|feel every/i, 'sensory claim'],
  [/unobstructed|without obstruction|clear sightlines|padded seat|drink service|in-seat drink/i, 'seat comfort or sightline claim'],
  [/360-degree|most authentic|best seats|best atmosphere|budget travellers|cultural explorers/i, 'forbidden persona or superlative copy'],
  [/surrounded almost entirely|gambling section|where gamblers/i, 'crowd or gambling characterisation'],
  [/selling fast|almost sold out|last chance|limited seats|nearly full|book before it sells/i, 'scarcity'],
...
for (const P of [{ key: 'hub', prefix: 'hub', kw: [...CL030, ...CL035] },
  ...
  for (const w of [1440, 1280, 1024, 860, 620, 390]) {
    ...
    const r = await p.evaluate(() => ({
      ...
      small: [...document.querySelectorAll('.mtx-np a.mtx-np__btn')].filter(a => a.offsetParent !== null)
  ...
  const nodes = await page.evaluate(() =>
    [...document.querySelectorAll('.mtx-np h1,.mtx-np h2,.mtx-np h3,.mtx-np p,.mtx-np summary,.mtx-np dt,.mtx-np dd,.mtx-np a')]
  ...
  if (P.key === 'dated') {
    const WRONG = ['15 July','22 September','29 September','1 October','2 October'];
    let bad = 0;
    for (const d of WRONG) if (visible.includes(d)) { bad++; fail(`wrong date on the dated page: "${d}"`); }
    const need = ['Wednesday 30 September 2026','17:00','18:00'];
    need.forEach(v => { if (!visible.includes(v)) fail(`required value missing: "${v}"`); });
    console.log(`   wrong dates: ${bad} (expected 0); 30 Sep, doors 17:00, start 18:00 all present: ${need.every(v => visible.includes(v))}`);
  }
  ...
  const links = await page.evaluate(() =>
    [...document.querySelectorAll('.mtx-np a')].map(a => ({ href: a.getAttribute('href'), text: a.textContent.trim() })));
  ...
  const imgs = await page.evaluate(() =>
    [...document.querySelectorAll('.mtx-np img')].map(i => ({ src: i.getAttribute('src'), alt: i.getAttribute('alt'),
  ...
  console.log(`   New Power logo: NOT PRESENT (no such asset exists in the project)`);

  console.log('\n-- headings --');
  const hs = await page.evaluate(() => [...document.querySelectorAll('.mtx-np h1,.mtx-np h2,.mtx-np h3')].map(h => h.tagName));
  ...
  const con = await page.evaluate(() => {
    ...
    return [...document.querySelectorAll('.mtx-np h1,.mtx-np h2,.mtx-np h3,.mtx-np p,.mtx-np a,.mtx-np dt,.mtx-np dd,.mtx-np summary')]
