...
for (const key of ['sep29','oct02']) {
  ...
  await p.route('**/static.tildacdn.com/**/10000339*.webp', r => {
    const id = (r.request().url().match(/(\d{10})\.webp/) || [])[1];
    const [label, col] = COLS[id] || ['?','#888'];
  ...
  const nodes = await p.$$('.mtx-kd > section, .mtx-kd > header');
