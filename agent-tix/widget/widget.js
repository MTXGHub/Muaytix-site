...
function mount(root, opts) {
  ...
  function statusMeta(st, seatsLeft){
    if(st === "available" || st === "limited"){
      return (seatsLeft > 0)
        ? {cls:"mtx-few", label:"Only " + seatsLeft + " left", live:true, go:false}
        : {cls:"mtx-go",  label:"Available",                   live:true, go:true};
    }
    if(st === "fully_booked")   return {cls:"mtx-full", label:"Fully booked",   live:false, go:false};
    if(st === "booking_closed") return {cls:"mtx-shut", label:"Booking closed", live:false, go:false};
    return {cls:"mtx-shut", label:"Closed", live:false, go:false};
  }
    ...
    return '<div class="mtx-tix-h"><h3>Booking closed with us</h3></div>' +
      '<div class="mtx-divert">' +
        ...
        '<a class="mtx-divert-go" href="' + esc(ev.divertUrl) + '" ' +
          ...
          '<svg width="15" height="15" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M6 3h7v7M13 3 4 12"/></svg>' +
        '</a>' +
  ...
  var CHEVRON =
    '<svg class="mtx-pick-go" width="17" height="17" viewBox="0 0 16 16" fill="none" ' +
  ...
  function pickButton(t){
    ...
    return '<button class="mtx-pick mtx-pick--' + (meta.live ? "on" : "off") + '" ' +
      'data-pick="' + esc(t.code) + '"' + (meta.live ? "" : " disabled") + '>' +
      '<span class="mtx-pick-bar"></span>' +
      (t.tagline ? '<span class="mtx-pick-why">' + esc(t.tagline) + '</span>' : "") +
      '<span class="mtx-pick-name">' + esc(t.name) + '</span>' +
      (unit != null
        ? '<span class="mtx-pick-price">' + esc(money(cur, unit)) + ' per ticket</span>'
        : "") +
      '<span class="mtx-state ' + meta.cls + '">' + esc(meta.label) + '</span>' +
      (meta.live ? CHEVRON : "") +
    '</button>';
  }
