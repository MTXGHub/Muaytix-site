...
function mount(root, opts) {
  ...
  function statusMeta(st, seatsLeft){
    if(st === "available" || st === "limited"){
      return (seatsLeft > 0)
        ? {tile:"mtx-avail--go", pill:"mtx-ok", label:"Only " + seatsLeft + " left", live:true}
        : {tile:"mtx-avail--go", pill:"mtx-ok", label:"Available",                   live:true};
    }
  ...
  function renderCal(){
    ...
    for(var d = 1; d <= days; d++){
      ...
      html += '<button class="mtx-day' + (state.date === key ? " mtx-sel" : "") +
              (ev.highlighted ? " mtx-hi" : "") + (opts.series ? " mtx-go" : "") +
  ...
  function pickButton(t){
    ...
    return '<button class="mtx-pick mtx-pick--' + (meta.live ? "on" : "off") + '" ' +
      ...
      (unit != null
        ...
        : "") +
      '<span class="mtx-avail ' + meta.tile + '">' + esc(meta.label) + '</span>' +
      (meta.live ? CHEVRON : "") +
    '</button>';
  }
  ...
  function classDetail(t){
    ...
    return '<section class="mtx-detail" style="--c:' + esc(t.colour || "#6E655C") + ';--ci:' + esc(t.ink || "#3A342D") + '">' +
      ...
      '<div class="mtx-detail-top">' +
        ...
        '<span class="mtx-pill ' + meta.pill + '"><i></i>' + meta.label + '</span>' +
      '</div>' +
